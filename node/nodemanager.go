...
func (nm *KNodeManager) LinkAfter(node *KNode, nodeType int, hint int) {
...
func (nm *KNodeManager) endPipeline(node *KNode, data []byte, dataFormat int, hint int) {
...
func (nm *KNodeManager) callNode(caller *KCallFrame, nodeDst *KNode, data []byte, dataFormat int, hint int) {
	...
	klog.D("PROCESSORR: Node:(%s | %d | %s), Fmt:%s, hint:%s:%d.", nodeDst.Type, nodeDst.typeId, nodeDst.Name, npName(dataFormat), npName(hint), hint)
...
func (nm *KNodeManager) SendtoNode(caller *KCallFrame, nodeDst *KNode, data []byte, dataFormat int, hint int) {
	if nodeDst.Processor != nil {
		go nm.callNode(caller, nodeDst, data, dataFormat, hint)
	}
}

func (nm *KNodeManager) getNexts(node *KNode, hint int) []*KNode {
...
// @data: data generated by node
func (nm *KNodeManager) sendtoSubs(caller *KCallFrame, nodeSrc *KNode, data []byte, dataFormat int, hint int) {
