...
func (nm *KNodeManager) callNode(caller *KCallFrame, nodeDst *KNode, data []byte, datafmt uint, hint uint) {
	...
	klog.D("PROCESSORR: Node:(%s | %d | %s), Fmt:%s, hint:%s:%d.", nodeDst.Type, nodeDst.TypeId, nodeDst.Name, NpStr(datafmt), NpStr(hint), hint)
...
func (nm *KNodeManager) sendtoSubs(caller *KCallFrame, nodeSrc *KNode, data []byte, datafmt uint, hint uint) {
	...
	for _, nodeNext := range nextNodes {
		...
		if nodeNext.Processor != nil && nodeNext != nodeSrc {
			go nm.callNode(caller, nodeNext, data, datafmt, hint)
		}
	}
}
